$ ->
	$ '.carousel'
		.slick
			...
			# waitForAnimate: true # Ignores requests to advance the slide while animating	$ '.carousel'
	...
	$ '.carousel-headers'
		.slick
			...
			# dotsClass: 'slick-dots' # Class for slide indicator dots container
			draggable: false # Enables desktop dragging
			# easing:  'ease' # animate() fallback easing
			# edgeFriction: 0.15 # Resistance when swiping edges of non-infinite carousels
			fade: true # Enables fade
			...
			# appendDots:#  $(element) # Change where the navigation dots are attached (Selector, htmlString, Array, Element, jQuery object)
			# mobileFirst: false # Responsive settings use mobile first calculation
			prevArrow: '<div class="arrow-l" />' #tml#jQuery selector) # object (DOM node#jQuery object) # <button type="button" class="slick-prev">Previous</button> # Allows you to select a node or customize the HTML for the "Previous" arrow.
			nextArrow: '<div class="arrow-r" />' #tml#jQuery selector) # object (DOM node#jQuery object) # <button type="button" class="slick-next">Next</button> # Allows you to select a node or customize the HTML for the "Next" arrow.
			# infinite: true # Infinite looping
			# initialSlide: 0 # Slide to start on
			# lazyLoad: 'ondemand' # Accepts 'ondemand' or 'progressive' for lazy load technique
			# pauseOnHover: true # Pauses autoplay on hover
